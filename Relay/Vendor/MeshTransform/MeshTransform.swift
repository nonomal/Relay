...
//
//  Source: submodules/TelegramUI/Components/MeshTransform/Sources/MeshTransform.swift
//          submodules/TelegramUI/Components/MeshTransform/MeshTransformApi/...
//
//  Drives the private `CAMutableMeshTransform`, which warps a layer through a vertex
//  mesh. Used by `RelayGlass` to bend the backdrop near the pane's edges, which is what
//  reads as optical refraction in Telegram's glass.
//
//  The mesh structs live in `MeshTransformApi.h` (bridging header) rather than being
//  translated to Swift: they cross into `@convention(c)` function pointers, and
//  Swift-declared structs are not Objective-C representable there.
//
